...
import { range, sum, values, fromPairs } from "lodash-es";
import {
  StackInputSourceCommon,
  stackInputSourceCommonSelector
} from "./stack-input-source-common";
...
export const winStackInputSourceSelector = createSelector(
  ...
  (
    ...
    const pairs = playerID.map(ID => {
      const selectedWin = winMap[ID];
      const src =
        selectedWin <= 0 || selectedWin == null ? restSrc : defaultSrc;
      return [ID, src];
    });
